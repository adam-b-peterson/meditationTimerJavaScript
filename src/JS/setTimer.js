...
function addSymbolToAnswer(string) {
  ...
  if (globalAnswer === currentValue) {
    //after "=" was pressed...
    console.log(
      'ORANGE-3a. after "=" pressed, just need one symbol added to currentValue'
    );
  ...
  if (symbols.includes(currentValue)) {
    //if a symbol was already pressed...
    ...
    console.log(
      "Orange-4b: replaced lastChar in newGlobalAnswer with",
      currentValue
    );
    return storeAndReset(newGlobalAnswer, symbolString);
  } else {
    //otherwise, adding new symbol to currentValue...
...
function storeAndReset(newGlobalAnswer, newCurrentValue) {
  ...
  console.log(
    "storeAndReset: replaced currentValue with symbol: ",
    newCurrentValue
  );
